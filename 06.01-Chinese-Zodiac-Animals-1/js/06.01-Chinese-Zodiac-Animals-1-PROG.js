...
let PreviousGuessNum=0;currentGuessNum=50; Minnum=1;Maxnum=100;guessTime=0;guessTimeAvg=0;
//targetNum=100;
//console.log('targetNum: ',targetNum);
for (let j=1;j<2;j++){
    let targetNum=Math.ceil((Math.random()*100));
    console.log('targetNum: ',targetNum);
    currentGuessNum=33; Minnum=1;Maxnum=100;
for (let i=1;i<=10;i++){
    
    
    //targetNum=Math.floor((Math.random()*100));
   // currentGuessNum=Math.floor((Minnum+Maxnum)/2);
    console.log('guess ' ,i, ' times   currentguess: ',currentGuessNum, '   maxnum ',Maxnum, '  minnum  ', Minnum);
    //for (let j=1;j<10;j++){
        if (currentGuessNum==targetNum){
            console.log('guess ' ,i, ' times to win');
            guessTime=guessTime+i;
            
            break;
        }
        else if (currentGuessNum>targetNum){
            //PreviousGuessNum=currentGuessNum;
            Maxnum=currentGuessNum;
            currentGuessNum=Math.floor((Maxnum+Minnum)/3);
            if (currentGuessNum==Math.floor((Maxnum+Minnum)/3))
                Math.floor((Maxnum+Minnum)/3*2);
            console.log('currentGuessNum>targetNum cur+pre ', currentGuessNum, ':::',Maxnum,'min ', Minnum) ;
        }
        else if (currentGuessNum<targetNum){
           // PreviousGuessNum=currentGuessNum;
           Minnum=currentGuessNum;
           currentGuessNum=Math.ceil((Maxnum+Minnum)/3*2);

           if (currentGuessNum==Math.ceil((Maxnum+Minnum)/3))
           Math.floor((Maxnum+Minnum)/3);
    ...
    console.log('game ' , j, " avg: ", guessTime/j);
}


    // let targetNum=Math.floor((Math.random()*100))+1;PreviousGuessNum=0;currentGuessNum=50; Minnum=1;Maxnum=100;
    // //targetNum=1;
    // console.log('targetNum: ',targetNum);
    // for (let i=1;i<=10;i++){
    //     //targetNum=Math.floor((Math.random()*100));
    //     currentGuessNum=Math.floor((Minnum+Maxnum)/2);
    //     console.log('guess ' ,i, ' times');
    //     //for (let j=1;j<10;j++){
    //         if (currentGuessNum==targetNum){
    //             console.log('guess ' ,i, ' times to win');break;
    //         }
    //         else if (currentGuessNum>targetNum){
    //             //PreviousGuessNum=currentGuessNum;
    //             Maxnum=currentGuessNum;
    //             currentGuessNum=Math.floor((Maxnum+Minnum)/2);
    //             console.log('currentGuessNum>targetNum cur+pre ', currentGuessNum, ':::',Maxnum,'min ', Minnum) ;
    //         }
    //         else if (currentGuessNum<targetNum){
    //            // PreviousGuessNum=currentGuessNum;
