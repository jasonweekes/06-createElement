...
// 9. Open **06.01-Creating-Elements-START.js** and get the **section** 
//    that serves as the parent wrapper of the interface:
...
for(let i = 0; i < 12; i++) {
    ...
    // 5.2. Make image elem for the sound icon
    let soundIcon = new Image();
    soundIcon.src = 'images/sound-icon.png';

    // 6.2. Assign class to sound icon:
    soundIcon.className = 'sound-icon';

    // 7.2. Assign eng property to the soundIcon:
    soundIcon.eng = animal.eng;

    // 8.2. Click the soundIcon to call playSound function:
    soundIcon.addEventListener('click', playSound);

    // 9.2. Output soundIcon to divvy:
    divvy.appendChild(soundIcon);

    // 14.2. Make image for Chinese character and set its src:
    const chineseChar = new Image();
    chineseChar.src = `images/chars/char-${animal.chi}.jpg`;

    // 15.2. Assign class to the Chinese char:
    chineseChar.className = 'chinese-char';

    // 16.2. Output Chinese character to the divvy:
    divvy.appendChild(chineseChar);

    // 17.2. Make a p-tag for holding years:
    const pTagYrs = document.createElement('p');

    // 18.2. Assign the p tag its class and output the p tag to divvy:
    pTagYrs.className = 'zodiac-year';
    divvy.appendChild(pTagYrs);

    // 19.2. Declare a start year variable and a string to hold all the years concatenated with spaces: "1880 1892 1904 1916 1928..."
    let startYr = animal.year - 156;
    let yearsStr = "";

    // 20.2. Run a for loop that starts 156 years before the current animal yaer and continues until the counter reaches the current animal year plus 12; counter goes up by 12 each time: 
    for(let y = startYr; y <= animal.year + 12; y += 12) {
        yearsStr += y + " ";
    }

    // 21.2. Output the yearsStr to the pTagYrs:
    pTagYrs.textContent = yearsStr;

    // 22.2. Make a span tag to hold English name and assign class:
    let english = document.createElement('span');
    english.className = 'english';

    // 23.2. Output eng name to english span and assign it id:
    english.innerHTML = animal.eng;
    english.id = 'eng' + i; // eng3 is the dragon

    // 24.2. Output span to divvy:
    divvy.appendChild(english);

    // 25.2, Make another span tag, this one for the pinyin spelling with tone markings.
    let pinyin = document.createElement('span');

    // 26.2 Output the pin and tone data as text:
    pinyin.innerHTML = `${animal.pin}<br>${animal.tone}`;

    // 27.2. Assign properties and css:
    pinyin.pin = animal.pin;
    pinyin.className = 'pinyin';
    pinyin.id = 'pin' + i;

    // 28.2. Output the pinyin span to the divvy div:
...
// 10.2. Instantiate an Audio object for playing sounds:
...
function playSound() {
    // 11.2. stop / pause any sound which may already be playing:
    sound.pause();
    // 12.2. set source of the .mp3 file (just like setting img src):
    sound.src = `audio/${this.eng}.mp3`; // chicken.mp3
    // 13.2. play the sound:
    sound.play();
}
